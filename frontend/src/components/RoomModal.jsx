...
const RoomModal = ({ room, onClose, onBooked }) => {
  ...
  return (
    <div className="fixed inset-0 z-[60] grid h-[100dvh] place-items-center overflow-y-auto bg-black/50 px-4 py-6 sm:p-6">
      <section className="my-auto max-h-[calc(100dvh-3rem)] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white shadow-xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 p-5">
          <div>
            <h2 className="text-2xl font-semibold text-sky-700">{room.name || room.roomNumber}</h2>
        ...
        <div className="space-y-5 p-5">
          <div className="overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
            <div className="relative aspect-video bg-anhdao-blue">
              ...
              {canNavigateImages && (
                <>
                  <button
                    aria-label={t('booking.previousImage')}
                  ...
                  <button
                    aria-label={t('booking.nextImage')}
            ...
            {images.length > 1 && (
              <div className="flex gap-2 overflow-x-auto bg-white p-3">
                {images.map((image, index) => (
                  ...
                  >
                    <img
                      alt={`${room.name || room.roomNumber} ${index + 1}`}
                      className="h-full w-full object-cover"
                      src={mediaUrl(image)}
                    />
                  </button>
          ...
          <div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-3">
            <div>
              <p className="text-xs font-semibold uppercase text-slate-500">{t('booking.standardOption')}</p>
              <p className="mt-1 text-xl font-bold text-slate-950">{formatCurrency(room.price)}</p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase text-slate-500">{t('common.status')}</p>
              <p className="mt-1 font-semibold text-slate-800">
                {isSoldOut ? t('booking.soldOut') : t(`common.${room.status}`)}
              </p>
            </div>
            <div>
              <p className="text-xs font-semibold uppercase text-slate-500">{t('common.type')}</p>
              <p className="mt-1 font-semibold text-slate-800">{t(`common.${room.type}`)}</p>
            </div>
          </div>
